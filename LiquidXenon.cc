...
TGraphErrors* MAD::LiquidXenon::EnrPE(Double_t lightYield)
{
   if (fEnrPE) return fEnrPE;
   ...
   Double_t x[n];
   for (Int_t i=0; i<n; i++) {
      x[i] = l[i] * lightYield * 122.;
   }
   Double_t dx[n]={0}, dy[n]={0};

   fEnrPE = new TGraphErrors(n,x,y,dx,dy);
   fEnrPE->SetTitle("");
   fEnrPE->GetXaxis()->SetTitle("Number of photoelectrons");
   fEnrPE->GetYaxis()->SetTitle("Nuclear recoil energy [keVnr]");
